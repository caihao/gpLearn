...
class LinearNet_Res(nn.Module):
    def __init__(self,input_channel,input_size_x,input_size_y,output_size,init_weights=False):
        ...
        b1 = nn.Sequential(nn.Conv1d(input_channel, 64, kernel_size=3, padding=1), nn.BatchNorm1d(64), nn.ReLU(), nn.MaxPool1d(kernel_size=3, padding=1))
        ...
        linear_input_size=self.features(x.reshape((1,input_channel,-1))).size(-1)
    ...
    def forward(self,x):
        x=self.features(x.reshape((1,self.input_channel,-1)))
        x=self.classifer(x)
        return x
