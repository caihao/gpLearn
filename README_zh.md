...
## 安装教程
1. 在您指定的位置上运行如下克隆命令：<br><center>**git clone https://github.com/caihao/gpLearn.git**</center>
...
| log_name | str | None | 指定日志文件文件名（请注意，如果设定的文件名与当前存在的日志文件名相冲突，程序可能会对当前设置的文件名作出相应修改） |
...
**loading_min_value** 参数设定了程序在加载数据时对像素点沉积电荷量的要求，仅仅符合条件的像素点被处理并标明在输入图像上，相关参数介绍如下：
| 关键字 | 类型 | 说明 |
| ---  | --- | --- |
| uniformThreshold | bool | 是否使用统一阈值 |
